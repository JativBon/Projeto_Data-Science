...
def collect_rows(datasets: list[str]) -> tuple[list[dict], list[str], dict[str, list[str]]]:
    ...
    for dataset in datasets:
        for phase, fallback_algo, template in EXPECTED_FILES:
            path = Path(template.format(dataset=dataset))
            if not path.exists():
...
def export_outputs(rows: list[dict], datasets: list[str], warnings: list[str], found_files: dict[str, list[str]]) -> tuple[Path, Path, Path, dict]:
    pd.DataFrame(rows).to_csv("validacao_ramex_multidataset.csv", index=False, encoding="utf-8", columns=[
    ...
    Path("validacao_ramex_multidataset.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    Path("validacao_ramex_multidataset.md").write_text(build_markdown(rows, datasets, warnings), encoding="utf-8")
    
    return Path("validacao_ramex_multidataset.csv"), Path("validacao_ramex_multidataset.json"), Path("validacao_ramex_multidataset.md"), summary
