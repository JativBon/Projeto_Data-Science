...
DATASETS_BASE = {
    "01": {"raw": "dataset 01.txt", "prep": "01_dataset.py", "seq": "sequencias_dataset01_limpo.txt"},
    "02": {"raw": "dataset 02.txt", "prep": "02_dataset.py", "seq": "sequencias_dataset02_limpo.txt"},
    "03": {"raw": "dataset 03.xlsx", "prep": "03_dataset.py", "seq": "sequencias_dataset03.txt"},
}
...
def get_files(ds_id: str, cfg: dict) -> dict[str, str]:
    ...
    return {
        ...
        "prep": cfg["prep"],
        "seq": cfg["seq"],
        "pairs": f"pares_frequencias_dataset{ds_id}.csv",
        "matrix": f"matriz_adjacencia_dataset{ds_id}.csv",
        "graph": f"grafo_dataset{ds_id}.png",
        "edges": f"grafo_edges_dataset{ds_id}.csv",
        "ramex_csv": f"ramex_dataset{ds_id}.csv",
        "ramex_png": f"ramex_dataset{ds_id}.png",
    }
...
def run_step(step_name: str, script_key: str, arg_keys: list[str], req_keys: list[str]) -> bool:
    ...
    for ds_id, cfg in DATASETS_BASE.items():
        ...
        script = files[script_key] if script_key in files else script_key
...
def run_validation() -> bool:
    reqs = ["08_validacao_comparativa.py"]
    for ds_id, cfg in DATASETS_BASE.items():
        files = get_files(ds_id, cfg)
        reqs.extend([files["edges"], files["ramex_csv"]])
    return run_command("Validacao comparativa final", [PYTHON, "08_validacao_comparativa.py"], reqs)
