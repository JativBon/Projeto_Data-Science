...
def main() -> None:
    ...
    parser.add_argument("--out", default="relatorio_conformidade_ramex.md", help="Ficheiro de saída.")
    parser.add_argument("--txt", action="store_true", help="Gerar também versão TXT simples.")
    args = parser.parse_args()

    base = Path(".")
    multi = load_json(base / "validacao_ramex_multidataset.json")
    
    per_dataset_count = sum(
        1 for ds in ["dataset01", "dataset02", "dataset03"] 
        if load_json(base / f"validacao_ramex_puro_{ds}.json")
    )
